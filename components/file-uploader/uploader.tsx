...
export function Uploader({ onChange, value, fileTypeAccepted }: iAppProps) {
  ...
  async function handleRemoveFile() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setFileState((prev) => ({
          ...prev,
          isDeleting: false, // fixed
          error: true,
        }));
        return;
      }

      // revoke only if local object URL
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: false,
    maxSize:
      fileTypeAccepted === "image" ? 10 * 1024 * 1024 : 5000 * 1024 * 1024, // 5mb calculation
  ...
  return (
    <Card
      {...getRootProps()}
      className={cn(
        "relative border-2 border-dashed transition-colors duration-200 w-full h-64 ease-in-out",
        isDragActive
